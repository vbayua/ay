<template>
  <section class="px-2 pt-32 bg-white md:px-0">
    <div class="container items-center max-w-6xl px-5 mx-auto space-y-6 text-center">
      <h1 class="text-4xl font-extrabold tracking-tight text-left text-gray-900 sm:text-5xl md:text-6xl md:text-center">
        <span class="block">Cheers to our precious moments <span class="block mt-1 text-purple-500 lg:inline lg:mt-0">Together</span></span>
      </h1>
      <p class="w-full mx-auto text-base text-left text-gray-500 md:max-w-md sm:text-lg lg:text-2xl md:max-w-3xl md:text-center">
        And onward to become best lovers beyond anyone could understand, together, understanding, loving, every day making each of our life better
      </p>
      <div class="relative flex flex-col justify-center md:flex-row md:space-x-4">
        <nuxt-link href="#_" to="/timeline" class="flex items-center w-full px-6 py-3 mb-3 text-lg text-white bg-purple-500 rounded-md md:mb-0 hover:bg-purple-700 md:w-auto">
          Next
          <svg
            xmlns="http://www.w3.org/2000/svg"
            class="w-5 h-5 ml-1"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            stroke-width="2"
            stroke-linecap="round"
            stroke-linejoin="round"
          >
            <line x1="5" y1="12" x2="19" y2="12" />
            <polyline points="12 5 19 12 12 19" />
          </svg>
        </nuxt-link>
      </div>
    </div>
    <div class="container items-center max-w-4xl px-5 mx-auto mt-16 text-center">
      <img src="https://cdn.devdojo.com/images/november2020/hero-image.png">
    </div>
    <section />
  </section>
</template>

<script>
export default {
  name: 'SecondComponent'
}
</script>
